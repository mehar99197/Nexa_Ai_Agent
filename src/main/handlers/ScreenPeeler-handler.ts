...
export default function registerScreenPeeler(): void {
  const triggerPeeler = async (): Promise<void> => {
    ...
    try {
      ...
      peelerWindow = new BrowserWindow({
        ...
        resizable: false,
        type: 'toolbar',
        // The peeler HTML is generated by us above and uses `require('electron')`
        // to call ipcRenderer.send directly, so nodeIntegration must stay on.
        // We block all navigation/new-window attempts below to keep the
        // attack surface limited to the static markup we just wrote.
        webPreferences: { nodeIntegration: true, contextIsolation: false }
      })

      // Deny any navigation away from the local file we just loaded — if the
      // injected HTML somehow tries to load a remote URL, we kill the window.
      peelerWindow.webContents.on('will-navigate', (e) => e.preventDefault())
      peelerWindow.webContents.setWindowOpenHandler(() => ({ action: 'deny' }))
  ...
  ipcMain.on('peeler-result', async (event, coordinates) => {
    ...
    try {
      ...
      resultWindow.webContents.on('will-navigate', (e) => e.preventDefault())
      ...
      try {
        ...
        const peelerAbort = new AbortController()
        const peelerTimeout = setTimeout(() => peelerAbort.abort(), 20_000)
          ...
          response = await fetch(
            'https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-lite:generateContent',
            {
              ...
              headers: {
                ...
                // Pass the key in a header so it doesn't leak through URL logs,
                // referers, or DevTools network traces.
                'x-goog-api-key': apiKey
              },
              signal: peelerAbort.signal,
              body: JSON.stringify({
                contents: [
                  {
                    parts: [
            ...
            if (parsed && typeof parsed === 'object') {
              detectedLanguage =
                typeof parsed.language === 'string' && parsed.language
                  ...
                  : 'javascript'
              extractedCode = typeof parsed.code === 'string' ? parsed.code : ''
            }
          } catch (_error) {
            extractedCode = typeof aiResponse === 'string' ? aiResponse : ''
          }
        } else {
